...
MODEL_PATH  = "/Users/yiming/Documents/Sunway/Sem 9/FYP2/Code/career_recommendation/training/models/lr/lr_tfidf_skill_model.pkl"
VECT_PATH   = "/Users/yiming/Documents/Sunway/Sem 9/FYP2/Code/career_recommendation/training/models/lr/tfidf_vectorizer.pkl"
LE_PATH     = "/Users/yiming/Documents/Sunway/Sem 9/FYP2/Code/career_recommendation/training/models/lr/label_encoder.pkl"
...
def recommend_from_resume(resume_text=None, skills_list=None, top_k=TOP_K):
    """
    Provide top-k job title recommendations for a single resume.
    Either pass `resume_text` (raw text) or `skills_list` (list of skill strings).
    Ensures feature count matches the trained model.
    """
    # Load artifacts
    ...
    # Load encoded CSV used during training to get skill columns and text column
    enc_df = pd.read_csv(ENCODED_CSV)

    # Detect text column used in training
    text_col = None
    for c in PREFERRED_TEXT_COLS:
        if c in enc_df.columns:
            ...
            break
    if text_col is None:
        # fallback: use first non-label column
        cand_cols = [c for c in enc_df.columns if c not in ("Job_Title", "Job_Label")]
        if not cand_cols:
            raise RuntimeError("Cannot infer skill/text columns from encoded CSV.")
        text_col = cand_cols[0]

    # Skill columns in **exact order** as during training
    ...
    # Build multi-hot skill vector in the **same order**
    ...
    if X.shape[1] < expected_features:
        ...
        X = hstack([X, csr_matrix(np.zeros((1, pad_width)))], format="csr")
    elif X.shape[1] > expected_features:
        # remove extra columns (rare, only if CSV has extra skills)
    ...
    results = list(zip(topk_labels, topk_probs.round(4)))
    return {
